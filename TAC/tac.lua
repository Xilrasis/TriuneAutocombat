...
-- ----------------------------------------------------------------------------
-- MacroQuest ImGui script. This repo IS a `TAC/` folder -- drop it directly
-- into <MacroQuest Root>/lua/ (giving you <MacroQuest Root>/lua/TAC/). Once
-- deployed, run the whole addon with: /lua run TAC (loads init.lua, which
-- requires this file) or run this file directly with: /lua run TAC/tac
-- Loads config/triune_data.lua (produced by extract_spells.py) -- checked
-- beside this script first, then your MQ config dir -- for the real,
-- era-correct spell + activated-AA lists. Saves your loadout to
...
do
    local paths = {}
    pcall(function()
        -- Shipped inside this TAC/ folder (TAC/config/triune_data.lua) -- the
        -- normal case for a repo checkout or release download.
        if scriptDir then table.insert(paths, scriptDir .. 'config/triune_data.lua') end
        -- Legacy/manual placements, checked as fallbacks.
        table.insert(paths, cfg .. '/triune_data.lua')
...
local COMBO_OPTIONS = {
    ...
    WHENS    = { 'HP <=', 'target HP <=', 'target HP between', 'my HP <=', 'my Mana <=', 'missing buff', 'missing pet',
        ...
        'add is loose', 'twist while fighting', 'in combat',
        'always' }
}
...
-- Parse one string as a class name; append its abbreviation to `found` if new.
local function addParsedClass(text, found)
...
-- Feed each line of a multi-line string to addParsedClass.
local function addParsedClassLines(text, found)
    if not text or text == '' or text == 'NULL' then return end
    for line in text:gmatch('[^\r\n]+') do
        addParsedClass(line, found)
    end
end
    ...
    pcall(function()
        ...
        if items and items > 0 then
            for i = 1, items do
                ...
                if ok then addParsedClass(text, found) end
            end
        end
    end)
    pcall(function() addParsedClassLines(node.Text(), found) end)
end
...
local function classesFromInventoryWindow(loud, force)
    ...
    pcall(function()
        ...
        if abbrChild and abbrChild() then
            addParsedClassLines(abbrChild.Text(), found)
        end
    end)
    ...
    if #found == 0 then
        pcall(function()
            ...
            if clsChild and clsChild() then
                addParsedClassLines(clsChild.Text(), found)
            end
        end)
    end
        ...
        pcall(function()
            ...
            if listChild and listChild() then
                ...
                if #found == 0 then
                    ...
                    if okText then addParsedClassLines(rawText, found) end
                end
            end
        end)
    end
...
local function createCastTracker()
    ...
    -- Called only as recordFailure(spellName, targetId, reason, maxRetries,
    -- lockoutSec, kind) -- targetId is a spawn id or nil, reason a string.
    -- (Previously carried a 4-way type-dispatch for alternate call shapes that
    -- nothing used, and which dropped `reason` on the no-target path.)
    ...
    -- Periodic housekeeping so a long camp session (no zoning, which would
    -- clear() everything) doesn't accumulate a per-spawn-id entry for every
    -- mob ever debuffed. Called from the main loop every ~30s.
    local function prune()
        local now = os.clock()
        for k, t in pairs(lockouts) do
            if now >= (tonumber(t) or 0) then lockouts[k] = nil end
        end
        for k, e in pairs(failureCount) do
            if (now - (tonumber(e.lastFail) or 0)) > 15.0 then failureCount[k] = nil end
        end
        for tid, m in pairs(targetLockouts) do
            if not isSpawnAlive(tid) then
                targetLockouts[tid] = nil
            else
                for sp, t in pairs(m) do
                    if now >= (tonumber(t) or 0) then m[sp] = nil end
                end
                if next(m) == nil then targetLockouts[tid] = nil end
            end
        end
        for tid in pairs(targetImmunities) do
            if not isSpawnAlive(tid) then targetImmunities[tid] = nil end
        end
    end
...
local function clearCursor()
    ...
    pcall(function()
        local count = 0
        while (mq.TLO.Cursor.ID() or 0) > 0 and count < 12 do
            mq.cmd('/autoinventory')
            count = count + 1
            if (mq.TLO.Cursor.ID() or 0) > 0 and count < 12 then mq.delay(50) end
        end
        if (mq.TLO.Cursor.ID() or 0) > 0 and ctrl and ctrl.debug_mode then
            print('\ay[Triune]\ax clearCursor: cursor still occupied after 12 tries (inventory full?).')
        end
    end)
    return true
end
...
-- Alias: isSpecialSkill and isActionSkill had identical bodies (membership in
-- any CLASS_ACTIONS list). Kept as a name so runtime.isSpecialSkill callers
-- still resolve.
local isSpecialSkill = isActionSkill
...
local function serialize(o, f, indent)
    local t = type(o)
    if t == 'number' then
        -- nan / inf render as tokens Lua can't load back ("nan", "inf"),
        -- which would corrupt the whole file. Coerce to 0.
        if o ~= o or o == math.huge or o == -math.huge then
            f:write('0')
        else
            f:write(tostring(o))
        end
...
local function savePreset(name)
    ...
    loadout.presets[name] = {
        name = name,
        gems = deepCopyTable(loadout.gems),
...
local function loadPreset(name, autoMem)
    if not name or name == '' then return false end
    ...
    if p.gems then
        loadout.gems = deepCopyTable(p.gems)
        saveLoadout(true)
        ...
        if autoMem or ctrl.automem then
            for i = 1, getNumGems() do
                local gg = loadout.gems[i]
                if gg and gg.spell then
                    runtime.pendingMem[i] = gg.spell
                end
            end
            print('\ag[Triune]\ax Queued spells to memorization bar.')
        end
        return true
    end
    return false
...
local function listPresets()
    ...
    if count == 0 then
        print('  \ayNo presets saved yet. Use /tac preset save <name> or the UI to save one.\ax')
    end
end
...
saveLoadout = function(silent)
    ...
    -- Write to a temp file and swap it in, so a crash mid-write or a serialize
    -- error can't truncate the real file and take every character's loadout
    -- with it (this runs every ~1.5s during play).
    local dest = cfg .. '/triune_loadout.lua'
    local tmp = dest .. '.tmp'
    ...
    if not ok then
        os.remove(tmp)
        print('\ar[Triune]\ax loadout save failed (serialize error) -- previous save kept.')
        return
    end
    if not os.rename(tmp, dest) then
        os.remove(dest) -- Windows os.rename won't clobber; retry after removing
        if not os.rename(tmp, dest) then
            os.remove(tmp)
            print('\ar[Triune]\ax loadout save failed (could not replace file).')
            return
        end
    end
...
local function onCharacterChanged()
    loadout = { gems = {}, aas = {}, discs = {}, actions = {}, clickies = {}, presets = {} }
    ctrl = defaultCtrl()
...
function UI.drawHelpTab()
    ...
    if ImGui.CollapsingHeader('Slash Commands', ImGuiTreeNodeFlags.DefaultOpen) then
        ...
        if ImGui.BeginTable('##HelpCmdTable', 2, tableFlags) then
            ...
            local commands = {
                { cmd = '/tac run / /tac start',                desc = 'Start / unpause auto-combat execution' },
                { cmd = '/tac pause / /tac stop',               desc = 'Pause auto-combat execution, halt movement & disengage pet' },
                { cmd = '/tac burn [on|off]',                  desc = 'Toggle burn mode (enables "Burn Only" spells, AAs, discs)' },
                { cmd = '/tac status',                         desc = 'Print current running state and combat mode to chat' },
                { cmd = '/tac compact / /tac mini',             desc = 'Toggle auto-resizing Compact Mini-Window HUD mode' },
                { cmd = '/tac help / /tac h',                   desc = 'Print slash command usage and command options in chat' },
                { cmd = '/tac clearcursor',                    desc = 'Clear item on cursor (autoinventory / drop / destroy per rules)' },
                { cmd = '/tac clear lockouts',                 desc = 'Clear all active spell lockouts, non-stacking buff backoffs, and mob immunities' },
                { cmd = '/tac zplane [5-100]',                 desc = 'Configure Hunter Tier 1 same-floor / Z plane height threshold (default 15)' },
                { cmd = '/tac huntz [10-300]',                 desc = 'Configure Hunter Tier 2 max vertical height difference (default 75)' },
                { cmd = '/tac <mode> [submode]',               desc = 'Switch combat mode (e.g. /tac manual, /tac puller hunt, /tac puller camp, /tac assist chase, /tac backline, /tac tank)' },
                { cmd = '/tac pullcon [tier] [on|off]',        desc = 'Configure Puller faction consideration filter (Scowling, Indifferent, etc.) or preset' },
                { cmd = '/tac wp [add|clear|del|on|off|list]', desc = 'Configure & toggle Puller Waypoint Patrol loop' },
                { cmd = '/tac pullhp [0-95]',                  desc = 'Configure minimum HP percentage threshold before pausing pulling to rest (default 0 / disabled)' },
...
function UI.getGemStatusBadge(i, g)
    ...
    if runtime.pendingMem and runtime.pendingMem[i] then
        return WARN, '[MEM*]', string.format('Queued to memorize "%s"', g.spell)
    end
    local memmedName = nil
    pcall(function() memmedName = mq.TLO.Me.Gem(i).Name() end)
    if not isGemMatching(i, g.spell) and (memmedName ~= g.spell) then
        return WARN, '[UNMEM]', string.format('Configured for "%s" but bar has "%s"', g.spell, tostring(memmedName or 'empty'))
    end

    local ready = false
    local timer = 0
    pcall(function() ready = mq.TLO.Me.SpellReady(i)() end)
    if not ready then
        pcall(function() timer = tonumber(mq.TLO.Me.GemTimer(i)()) or 0 end)
        if timer > 0 then
    ...
    return GOOD, '[RDY]', 'Spell is memorized and ready to cast.'
end
        ...
        for i = 1, maxGems do
            ...
            if i > 1 then
                if ImGui.SmallButton('^##u') then
                    ...
                    gemsTable[i] = tmp
                    if ctrl.automem and isActiveSet then
                        if gemsTable[i - 1] and gemsTable[i - 1].spell then runtime.pendingMem[i - 1] = gemsTable[i - 1].spell end
                        if gemsTable[i] and gemsTable[i].spell then runtime.pendingMem[i] = gemsTable[i].spell end
                    end
                    saveLoadout(true)
                end
...
function UI.drawGemTabHeader(gemsTable)
    ...
    ImGui.SameLine()
    ctrl.automem = ImGui.Checkbox('Auto-mem', ctrl.automem)
    if ImGui.IsItemHovered() then
    ...
    if ImGui.IsItemHovered() then
        ImGui.SetTooltip('Read active spell bar into this loadout.')
    end

    ImGui.SameLine()
    local pendingCount = 0
    for _ in pairs(runtime.pendingMem) do pendingCount = pendingCount + 1 end
    if ImGui.SmallButton('Mem All') then
        if mq.TLO.Me.Combat() then
            print('\ay[Triune]\ax cannot mem in combat -- wait until combat ends, then click Mem All again.')
        else
            for i = 1, getNumGems() do
    ...
    ImGui.SameLine()
    local presetNames = { '-- Load Preset --' }
    if loadout.presets then
        for k in pairs(loadout.presets) do presetNames[#presetNames + 1] = k end
        table.sort(presetNames, function(a, b)
            if a == '-- Load Preset --' then return true end
            if b == '-- Load Preset --' then return false end
    ...
    if ImGui.SmallButton('Save') then
        local pName = (runtime.presetInput or ''):gsub('^%s+', ''):gsub('%s+$', '')
        if pName ~= '' then
            savePreset(pName)
            runtime.presetInput = ''
        else
            print('\ay[Triune]\ax Please enter a preset name first.')
        end
    end
    if ImGui.IsItemHovered() then ImGui.SetTooltip('Save current loadout under entered name.') end

    if pChoice > 1 then
        ImGui.SameLine()
        if ImGui.SmallButton('Del') then
            deletePreset(presetNames[pChoice])
        end
...
function UI.drawCooldownsTab()
    ...
    ImGui.BeginChild('##cooldownsBody', 0, 0)
...
-- Shared button palettes so the main controls and the Mini HUD's
-- Start / Pause / Burn buttons look the same. Each returns a push count for
-- UI.popBtnPalette().
function UI.pushBtnPalette(bR, bG, bB, hR, hG, hB, aR, aG, aB, whiteText)
    local Col = ImGuiCol or _G.ImGuiCol or (mq.imgui and mq.imgui.Col)
    if not Col then return 0 end
    local n = 0
    if pcall(ImGui.PushStyleColor, Col.Button, bR, bG, bB, 1.0) then n = n + 1 end
    if pcall(ImGui.PushStyleColor, Col.ButtonHovered, hR, hG, hB, 1.0) then n = n + 1 end
    if pcall(ImGui.PushStyleColor, Col.ButtonActive, aR, aG, aB, 1.0) then n = n + 1 end
    if whiteText and pcall(ImGui.PushStyleColor, Col.Text, 1.0, 1.0, 1.0, 1.0) then n = n + 1 end
    return n
end
function UI.popBtnPalette(n)
    if n and n > 0 then pcall(ImGui.PopStyleColor, n) end
end
function UI.pushPausePalette() -- green: engine running, click to pause
    return UI.pushBtnPalette(0.12, 0.55, 0.22, 0.18, 0.70, 0.28, 0.08, 0.40, 0.15, true)
end
function UI.pushStartPalette() -- red: engine paused, click to start
    return UI.pushBtnPalette(0.65, 0.15, 0.15, 0.80, 0.22, 0.22, 0.50, 0.10, 0.10, false)
end
function UI.pushBurnOnPalette() -- pulsing red while Burn is active
    local p = (math.sin(os.clock() * 8.0) + 1.0) * 0.5
    local r, g, b = 0.50 + 0.45 * p, 0.05 + 0.08 * p, 0.05 + 0.08 * p
    return UI.pushBtnPalette(r, g, b,
        math.min(1.0, r + 0.15), math.min(1.0, g + 0.15), math.min(1.0, b + 0.15),
        0.70, 0.0, 0.0, true)
end

function UI.drawActionControls()
    if ctrl.running then
        local n = UI.pushPausePalette()
...
function UI.drawControlTab()
    ...
    ImGui.BeginChild('##controlBody', 0, 0)
...
function UI.drawSettingsTab()
    ...
    ImGui.BeginChild('##settingsBody', 0, 0)
...
local function drawMiniGui()
    ...
    if show then
        ...
        if ctrl.mode == 'Assist' then
            ...
            ctrl.ma_name = ImGui.InputText('MA Name##miniMaName', ctrl.ma_name or '')
            if ImGui.IsItemHovered() then
                UI.setTooltip('Character to assist. Leave blank to assist the group leader.')
            end
        end
        ...
        -- Row 2: Action Controls Toolbar (Start/Pause, Burn) -- same palettes
        -- as the full-window controls, see UI.push*Palette().
        if ctrl.running then
            local n = UI.pushPausePalette()
            if ImGui.Button('PAUSE##miniRunBtn', 90, 22) then
                if ctrl.mode == 'Manual' then
                    setManualHunterPetHold(true, true)
                else
                    setManualHunterPetHold(false, true)
            ...
            local n = UI.pushStartPalette()
            if ImGui.Button('START##miniStartBtn', 90, 22) then
                if ctrl.use_waypoints and ctrl.waypoints and #ctrl.waypoints > 0 then
                    runtime.setNearestWaypoint()
                end
                ctrl.running = true
        ...
        if ctrl.burn then
            local n = UI.pushBurnOnPalette()
            if ImGui.Button('BURN (ON)##miniBurnBtn', 90, 22) then
                ctrl.burn = false
            end
            UI.popBtnPalette(n)
        else
            if ImGui.Button('BURN (OFF)##miniBurnBtn', 90, 22) then
                ctrl.burn = true
            end
        end
        if ImGui.IsItemHovered() then
...
function UI.getTrackedCooldownItems()
    ...
    if loadout and loadout.gems and (ctrl.cooldown_category == 'Spells' or ctrl.cooldown_category == 'All') then
        for slot, g in pairs(loadout.gems) do
            if g and g.enabled and g.name and g.name ~= '' then
                local isReady = false
                local timerSec = 0
                pcall(function()
...
-- Curse and Corruption share one detection shape across Me, NetBots, the
-- current Target, and group members -- only the TLO member names differ.
-- `kind` is 'Curse' or 'Corruption'; `A.flag` is the boolean-ish TLO member
-- (Me.Cursed / Target.Corrupted / ...) and `A.counter` the numeric one
-- (Me.CountersCurse / ...). (Poison/Disease is handled separately -- it has
-- no CounterType buff scan and an extra Debuffs-plugin path.)
local COUNTER_AFFLICTION = {
    Curse      = { flag = 'Cursed',    counter = 'CountersCurse' },
    Corruption = { flag = 'Corrupted', counter = 'CountersCorruption' },
}
local function hasCounterAffliction(targetId, kind)
    if not targetId or targetId <= 0 then return false end
    local A = COUNTER_AFFLICTION[kind]
    if not A then return false end

    if targetId == mq.TLO.Me.ID() then
        local hit = false
        pcall(function()
            if mq.TLO.Me[A.flag] and mq.TLO.Me[A.flag]() then hit = true end
            if mq.TLO.Me[A.counter] and (tonumber(mq.TLO.Me[A.counter]()) or 0) > 0 then hit = true end
        end)
        ...
        pcall(function()
            for i = 1, (mq.TLO.Me.CountBuffs() or 0) do
                ...
                if b and b() and (b.CounterNumber() or 0) > 0 and b.CounterType() == kind then
                    n = n + 1
                end
            end
        end)
    ...
    if cleanName ~= '' then
        ...
        pcall(function()
            local nb = mq.TLO.NetBots(cleanName)
            if nb and nb() then
                nbHit = tonumber(nb[A.flag]()) or 0
                if nbHit == 0 and nb[A.counter] then
                    nbHit = tonumber(nb[A.counter]()) or 0
                end
                if nbHit == 0 then
                    local det = tostring(nb.Detrimental() or '')
                    if det:find(kind) then nbHit = 1 end
                end
            end
        end)
        if nbHit > 0 then return true end
    end

    if (mq.TLO.Target.ID() or 0) == targetId then
        local tc = false
        pcall(function()
            local c = mq.TLO.Target[A.flag]
            if c and c() then
    ...
    for i = 0, total do
        ...
        if m and m() and (m.ID() or 0) == targetId then
            local mc = false
            pcall(function()
                if m[A.flag] and m[A.flag]() then mc = true end
                if m[A.counter] and (tonumber(m[A.counter]()) or 0) > 0 then mc = true end
            end)
...
local function isCursed(targetId) return hasCounterAffliction(targetId, 'Curse') end
local function isCorrupted(targetId) return hasCounterAffliction(targetId, 'Corruption') end
...
function runtime.conditionMet(when, pct, spellName, targetId, cls, token, extra)
    ...
    -- 'ally is Dead' was removed from the trigger menu: resolveTargetId()
    -- rejects dead/corpse targets before conditionMet() is ever reached, so it
    -- could never fire. Kept inert here so any stale saved entry stays a no-op
    -- (rather than falling through to the permissive `return true` below).
    if when == 'ally is Dead' then return false end
...
function runtime.castGem(i, g, id)
    ...
    if g.cls == 'Brd' and (wasAttacking or (ctrl and ctrl.combat_style == 'Melee')) and not mq.TLO.Me.Combat() then
        mq.cmd('/attack on')
    end
    return true
end
...
runtime.useClickie = function(c, id)
    ...
    if castMs == 0 and (wasAttacking or (ctrl and (ctrl.combat_style or 'Melee') == 'Melee')) and not mq.TLO.Me.Combat() then
        mq.cmd('/attack on')
    end
    return true
end
...
runtime.fullStop = function()
    ...
    -- (radio button, /tac style command) -- that's a real user choice, unlike
    -- a pause/stop.
    if isCasting() then
        mq.cmd('/stopsong')
        mq.cmd('/stopcast')
...
runtime.onZoned = function()
    ...
    runtime.conCache = {} -- faction cons are zone-specific (city guards etc.)
...
-- Out-of-combat rest (Med Break). Extracted from combatTick(). Returns
-- true when a med break is holding and the tick should stop here.
function runtime.medBreakTick()
...
local function combatTick()
    ...
    if runtime.medBreakTick() then return end
...
local function triuneCommand(...)
    ...
    elseif cmd == 'help' or cmd == 'h' or cmd == '?' then
        print('\ag[Triune]\ax --- Slash Commands (/tac or /triune) ---')
        print('  \ag/tac run | start\ax - Start autocombat execution')
        print('  \ag/tac pause | stop\ax - Pause execution & disengage combat')
        print('  \ag/tac burn [on|off]\ax - Toggle burn mode')
        print('  \ag/tac debug\ax - Toggle live combat debug telemetry in chat')
        print('  \ag/tac status\ax - Print running state and mode')
        print('  \ag/tac compact | mini | hud\ax - Toggle compact HUD mode')
        print('  \ag/tac help | h | ?\ax - Print slash command summary')
        print('  \ag/tac clearcursor | autoinv\ax - Clear items from cursor')
        print('  \ag/tac style [melee|ranged|spell]\ax - Configure combat style')
        print('  \ag/tac range [dist]\ax - Configure melee or ranged distance')
        print('  \ag/tac cd | cooldowns\ax - Toggle popout Cooldown & Ability Monitor window')
        print('  \ag/tac zplane [5-100]\ax - Configure Hunter Tier 1 same-floor / Z plane height threshold')
        print('  \ag/tac huntz [10-300]\ax - Configure Hunter Tier 2 max vertical height difference')
        print('  \ag/tac pullcon [con]\ax - Configure faction consideration filter')
        print('  \ag/tac wp [add|clear|del|on|off|list]\ax - Configure & toggle Puller Waypoint Patrol')
        print('  \ag/tac pullhp [0-95]\ax - Set minimum HP % threshold before pausing pulling to rest')
        print('  \ag/tac clear lockouts\ax - Clear all active spell lockouts & mob immunities')
        print(
            '  \ag/tac <mode> [submode]\ax - Switch combat mode (manual, puller [hunt|camp], assist [chase|camp|backline])')
    ...
    elseif cmd == 'clearlockouts' or cmd == 'unlock'
        or (cmd == 'clear' and (normalizeCommandKey(args[2]) == 'lockouts'
            or normalizeCommandKey(args[2]) == 'locks'
            or normalizeCommandKey(args[2]) == 'all')) then
        if castTracker and castTracker.clear then
            castTracker.clear()
            print('\ag[Triune]\ax Cleared all active spell lockouts, target backoffs, and mob immunities.')
        end
    ...
    elseif cmd == 'pullcon' or cmd == 'con' or cmd == 'confilter' then
        ...
        if arg2 == 'preset' then
            ...
            elseif arg3 == 'clear' or arg3 == 'none' then
                ...
                print('\ay[Triune]\ax usage: /tac pullcon preset [all|hostile|indifferent|none]')
            end
            saveLoadout(true)
        elseif arg2 ~= '' then
            local targetCon = nil
            ...
            print(
                '\ay[Triune]\ax usage: /tac pullcon [con_name] [on|off] OR /tac pullcon preset [all|hostile|indifferent|none]')
        end
    elseif cmd == 'wp' or cmd == 'waypoint' or cmd == 'waypoints' then
        ...
        elseif sub == 'delete' or sub == 'del' or sub == 'remove' then
            ...
            if idx and runtime.wpDelete(idx) then
                ...
                print('\ay[Triune]\ax usage: /tac wp delete [index]')
            end
        ...
        elseif sub == 'radius' or sub == 'arrival' then
            ...
            if r and r >= 5 and r <= 100 then
                ...
                print('\ay[Triune]\ax usage: /tac wp radius [5-100]')
            end
        elseif sub == 'scan' or sub == 'scanradius' then
            ...
            if r and r >= 20 and r <= 500 then
                ...
                print('\ay[Triune]\ax usage: /tac wp scan [20-500]')
            end
        elseif sub == 'list' or sub == 'show' or sub == '' then
            ...
            if #ctrl.waypoints == 0 then
                print('  \ayNo waypoints defined. Use /tac wp add [name] to add locations.\ax')
            else
            ...
            print(
            '\ay[Triune]\ax usage: /tac wp [add [name]|clear|delete [idx]|on|off|toggle|radius [5-100]|scan [20-500]|list]')
        end
    elseif cmd == 'style' or cmd == 'combatstyle' then
        ...
        elseif st == 'spell' or st == 'cast' or st == 'caster' then
            ...
            print('\ay[Triune]\ax usage: /tac style [melee|ranged|spell]')
        end
    elseif cmd == 'range' or cmd == 'meleerange' or cmd == 'dist' then
        ...
        if val then
            ...
            if ctrl.combat_style == 'Melee' then
                print(string.format('\ag[Triune]\ax Current Max Melee Distance: %d units. (usage: /tac range [5-50])', ctrl.melee_dist or 14))
            else
                print(string.format('\ag[Triune]\ax Current Ranged Distance: %d units. (usage: /tac range [5-200])', ctrl.ranged_dist or 40))
            end
        end
    elseif cmd == 'huntz' or cmd == 'z' then
        ...
        if val then
            ...
            print(string.format('\ag[Triune]\ax Current Hunter Max Height Diff (Z): %d units. (usage: /tac huntz [10-300])', ctrl.hunter_z or 75))
        end
    elseif cmd == 'zplane' or cmd == 'huntplane' or cmd == 'floorz' then
        ...
        if val then
            ...
            print(string.format('\ag[Triune]\ax Current Hunter Floor Height (Z Plane): %d units. (usage: /tac zplane [5-100])', ctrl.hunter_z_plane or 15))
        end
    elseif cmd == 'pullhp' or cmd == 'minhp' then
        ...
        if val then
            ...
            print(string.format('\ag[Triune]\ax Current Min Pull HP threshold: %d%%. (usage: /tac pullhp [0-95])', ctrl.pull_min_hp_pct or 0))
        end
    elseif cmd == 'preset' or cmd == 'loadout' then
        ...
        elseif sub == 'list' or sub == '' then
            ...
            print('\ay[Triune]\ax usage: /tac preset [save <name>|load <name>|delete <name>|list]')
        end
    elseif setTriuneMode(args[1], args[2]) then
        return
    else
        print(
            '\ay[Triune]\ax usage: /tac [run|pause|burn|compact|status|clearcursor|style|range|zplane|huntz|pullhp|preset|help|pullcon|wp|manual|puller [hunt|camp]|assist [chase|camp|backline]]')
    end
end
...
mq.imgui.init('TriuneCooldownWindow', drawCooldownWindow)
...
print('\ag[Triune]\ax loaded v' ..
    ...
    '. Use /tac run | /tac pause | /tac status | /tac <mode>. /lua stop TAC to exit.')
if not navLoaded() then
...
local function runMainLoop()
    while open do
        ...
        -- periodic housekeeping: drop cast-tracker entries for dead/despawned mobs
        if castTracker.prune and (os.clock() - (runtime.lastTrackerPruneAt or 0)) > 30.0 then
            runtime.lastTrackerPruneAt = os.clock()
            pcall(castTracker.prune)
        end
...
-- Entry point. MUST run in the script coroutine, never during module import
-- (mq.delay cannot yield mid-require and will hang the client). init.lua sets
-- __TAC_LAUNCHED_VIA_INIT and calls this after require() returns; a direct
-- `/lua run TAC/tac` has no init.lua, so we start ourselves at chunk end.
local function start()
    mq.delay(1) -- yield once so we're cleanly in the coroutine before the loop
    if not navLoaded() then
        mq.cmd('/plugin mq2nav')
        mq.delay(250)
    end
    print('\ag[Triune]\ax started v' .. VERSION .. ' -- /tac help for commands. /lua stop TAC to exit.')
    runMainLoop()
    if runtime.clearMapRadiusVisuals then runtime.clearMapRadiusVisuals() end
    saveLoadout(true)
end
...
if not _G.__TAC_LAUNCHED_VIA_INIT then
    start()
end
