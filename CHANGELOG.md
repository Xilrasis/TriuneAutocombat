...
- **Cast-tracker no longer grows unbounded during a long camp (v1.7.15):** it keyed a target-lockout / immunity entry per spawn id and only ever fully cleared on zone/death/manual command. Added `castTracker.prune()` — called from the main loop every ~30s — that drops entries for dead or despawned mobs and expired global lockouts. Also clear `conCache` on zone (faction cons are zone-specific anyway). `os.clock()` audit: it's wall-clock on Windows (MacroQuest is Windows-only), so the engine's timer basis is fine as-is — no change.

- **UI: restored some changes lost in the restructure (v1.7.14):**
  - Full window: the header, Start/Pause/Burn controls, and the tab bar are now pinned to the top. Each tab's content scrolls inside its own region, so a long tab (Settings especially) no longer pushes the tabs and controls off screen.
  - Mini HUD: the **MA Name** field now shows while in Assist mode.
  - Mini HUD: the Pause and Burn buttons now use the same palettes as the full-window controls (green Pause, pulsing-red Burn ON). Start/Pause/Burn button styling is shared via `UI.push*Palette()` so the two views stay in sync.

- **Internal cleanup, no behavior change (v1.7.13):** `isCursed()` and `isCorrupted()` were near-identical ~70-line functions differing only in TLO member names (`Cursed`/`CountersCurse` vs `Corrupted`/`CountersCorruption`). Merged into `hasCounterAffliction(id, kind)` with a small name table. `isPoisonedOrDiseased()` left as-is — it has a genuinely different shape (Debuffs-plugin path, no CounterType buff scan).

- **Internal cleanup, no behavior change (v1.7.12):**
  - `classesFromInventoryWindow` / `scanOneNode`: the "parse a line → dedupe into found[]" block was copy-pasted 6×. Factored into `addParsedClass()` / `addParsedClassLines()` (~60 fewer lines).
  - `recordFailure()` (cast-lockout system): removed a 4-way type-dispatch that emulated call shapes nothing used — and which silently dropped the failure `reason` on the no-target path (so e.g. a no-target "resisted" was treated as generic). Now a plain 6-arg signature. Test copy updated to match.

- **Loadout saves are now crash-safe (v1.7.11).** `saveLoadout()` (runs every ~1.5s during play) writes to `triune_loadout.lua.tmp` and swaps it in, and wraps serialization in `pcall`. A crash mid-write or a serialize error can no longer truncate `triune_loadout.lua` and wipe every character's loadout.
- **`serialize()` handles nan/inf (v1.7.11).** A non-finite number (e.g. a bad coordinate from an MQ read) used to render as `nan`/`inf` — tokens Lua can't load back — corrupting the whole file. Now coerced to `0`.
- **`clearCursor()` can no longer freeze the client for ~13s (v1.7.11).** The retry loop was capped at 255 × 50ms. Now 12 tries (~0.6s), with a debug note if the cursor is still stuck (inventory full).

- **Removed the non-functional "ally is Dead" trigger (v1.7.10).** It was in every When menu but `resolveTargetId()` rejects dead/corpse targets before the condition is evaluated, so a spell keyed to it could never fire. Removed from the menu; kept inert for any stale saved entry. (Auto-rez would need a dedicated dead-ally target resolver — a separate feature, not a fix.)
- **Removed the dead "wander" code path (v1.7.10).** `pursuit.wanderLoc` / `pursuit.wanderSince` were declared and cleared in ~10 places but never assigned a real value; the one branch that read `wanderLoc` was therefore unreachable. All gone.

- **Removed the Status tab (v1.7.9).** `UI.drawStatusTab()` and its only-caller helper `getConColorRgb()` are gone (~625 lines). The tab was a read-only mirror of state already visible on the Control/Cooldowns tabs, the header bar, and the in-game target/XTarget windows; its 1-click target actions (Face / Attack / Clear / pull-list / ignore-list) remain available elsewhere. `drawStatusProgressBar()` is kept — the Cooldown Monitor uses it.
- **Slash-command fix: `/tac clear lockouts` now works.** The branch tested an undefined `arg` variable (always nil), so only `/tac clearlockouts` / `/tac unlock` fired. It now reads the actual second argument.
- **Cleanup (no behavior change):**
  - `isSpecialSkill()` was a byte-for-byte copy of `isActionSkill()`; it is now an alias.
  - Removed a dead second definition of `isXTargetId()` (a later re-assignment fully shadowed the first — kept one).
  - Removed a duplicate `runtime.desiredRange` / `runtime.maxMeleeDistance` binding.
  - `onCharacterChanged()` now seeds `loadout.presets` like the other initializers.
  - `README.md` download/links repointed from the upstream repo to this fork; Status-tab feature section removed.

## 2026-09-02

- **Fixed the folder entry point — `/lua run TAC` now actually starts (v1.7.8).**
  - `init.lua`'s `require('tac')` (added in the `TAC/` re-wrap) ran `tac.lua`'s blocking main loop *during module import*, which either threw `Cannot delay while importing a module` or hard-froze the client (`mq.delay` busy-waits MacroQuest's main thread when it cannot yield). `tac.lua` now defines `start()` (mq2nav auto-load + main loop + shutdown cleanup) and `return`s `{ start = start }` instead of looping at chunk end; it self-starts only when run directly via `/lua run TAC/tac`. `init.lua` `require`s the module (loads with no blocking) then calls `tac.start()` at top level, in the script coroutine where `mq.delay` yields correctly. The module-scope `mq.delay(250)` mq2nav load moved into `start()`.
- **Renamed the `/ac` slash command to `/tac`.**
  - The primary bind is now `/tac` (`/triune` and `/triunerun` are unchanged). Updated every in-game help/usage string, the Help tab's Slash Commands table, and the load banner. `/ac` is no longer bound — update existing keybinds and macros to `/tac`.
  - Docs: `README.md` Slash Commands table and prose updated; `/lua stop triune` corrected to `/lua stop TAC` in the load banner. Existing `/ac` keybinds and macros must be re-pointed at `/tac`.
- **CI workflows caught up with the `TAC/` restructure.** `ci.yml` version-sync/luacheck/unit-test steps and `release.yml` packaging now target the real post-restructure paths (they had been left pointing at the pre-restructure layout, so `main` CI was red); added a `TAC/tests/**` luacheck override for dev-only test scaffolding.

- **Fork Standalone Cleanup: Removed Bonus Tools & Flattened Repo Layout.**
  - **Context**: This fork is no longer PR'd back to the upstream repo (upstream `main` is kept as a tracked remote for potential future cherry-picking, but this fork now develops directly on `main`). The repo has been trimmed to just the core engine, its data, and documentation, and the repo root itself is now flat rather than wrapped in a `TAC/` folder.
  - **Removed Standalone Tools**: Deleted `triune_map.lua` (2D Map & Norrath Atlas), `triune_spellbook.lua` (Spellbook Browser), `triune_dps.lua` (DPS Parser), `triune_cursor.lua` (Cursor Manager), `triune_updater.lua` (in-game Release Updater), `triune_buffbot.lua` (Interactive Buffbot), `triune_track.lua` (Zone NPC Tracker — this had already been dead code from `triune.lua`'s perspective; its slash commands actually launched `triune_map.lua`), and `triune_buttons.lua` (standalone ButtonMaster-style hotbar, never wired into `triune.lua` at all).
  - **Removed Updater Plumbing**: Deleted `triune_updater.py`, `update.bat`, `update.sh`.
  - **Removed Bundled Resources**: Deleted `resources/` in its entirety (`ItemDB.txt`, `Zones.ini`, and the bundled `MQ2Nav/*.navmesh` files) — confirmed via full-repo grep to be unreferenced by any Triune `.lua` file.
  - **Removed Misc**: Deleted the empty, accidentally-tracked `err.log`, and removed `config/ingame.cfg` from the repo (the corresponding local/live file is left untouched).
  - **`triune.lua` Wiring Cleanup**: Removed every UI button, tooltip, Help-tab row, print-based help line, usage-string token, and slash-command branch (`/ac spellbook`, `/ac cursorui`, `/ac buffbot`, `/ac dps` / `/dps`, `/ac update`, `/ac map` / `/ac track`) that referenced the removed tools, and removed the now-unused `toggleTool()` helper. `/ac clearcursor` (core cursor-clear action) and `/ac clear lockouts` were kept — only the standalone tool windows and their commands were removed.
  - **New Entry Point (`init.lua`)**: Added a small `init.lua` at the repo root that `require('triune')`s the engine, so the addon can be launched as a folder (`/lua run TAC`, once deployed under that name), while `triune.lua` remains independently runnable via `/lua run TAC/triune`. Not yet verified in-game.
  - **Flattened Repo Root (no `TAC/` folder in the repo)**: `init.lua`, `triune.lua`, and `config/triune_data.lua` now live directly at the repo root — there is no `TAC/` wrapper folder in the repo itself. `tests/test_pure_logic.lua` stays at its original top-level path. `.luacheckrc` and `.luarc.json` moved into a new `util/` folder (dev-only tooling config, not shipped) as `util/.luacheckrc` / `util/.luarc.json`. Updated every path reference accordingly (`ci.yml`, `release.yml`, `test_pure_logic.lua`'s `readFile` calls, `.gitattributes`, README, AGENTS.md).
  - **Release Packaging Preserves the Install UX**: Even though the repo is flat, `.github/workflows/release.yml` still packages `init.lua` + `triune.lua` + `config/` into a `TAC/` folder inside the shipped zip (alongside README/CHANGELOG), so a release download still extracts to `<MacroQuest Root>/lua/TAC/` exactly as before. Running from a source checkout instead means copying those three items into a folder you name `TAC` yourself.
  - **Test Suite Pruning**: Removed the `test_pure_logic.lua` cross-module suites that exercised the deleted tool files (`triune_dps.lua`, `triune_buffbot.lua`, `triune_map.lua`, `triune_updater.lua`), and removed `tests/check_theme_consistency.sh` (it compared `pushTheme()` copies across the now-deleted satellite tools and no longer has anything to check).
  - **CI/Release Fixes (`.github/workflows/`)**: In `ci.yml`, removed the `theme-consistency` job, dropped `triune_updater.lua` from the `version-sync` job's version-drift check, updated the `luacheck` step to run `luacheck . --config util/.luacheckrc`, and fixed the syntax-check/unit-test steps' file paths for the flattened layout. In `release.yml`, collapsed the two release archives into one (`TriuneAutocombat.zip`) built via a staging directory (rather than a single `git archive` prefix) so it can mix a `TAC/`-prefixed runtime subset with root-level README/CHANGELOG, and rewrote the install/update instructions to drop the old in-game-Updater/`update.bat`/`update.sh` steps, since those no longer exist.
  - **Docs**: Updated `README.md` (Built-in Bonus Tools table, Slash Commands table, File Structure tree, run commands) and `.agents/AGENTS.md` (File Map, Standalone File Rule historical note, version-sync note, binary-files note, one code-pattern reference, run command) to drop references to the removed tools/files and reflect the flattened layout.

- **Renamed `triune.lua` → `tac.lua`, and re-wrapped everything in a `TAC/` folder for trivial drop-in installs.**
  - **Rename**: `triune.lua` is now `tac.lua`. Updated every self-reference (header comments, run-command examples) and every external reference to it: `init.lua`'s `require('triune')` → `require('tac')`, `tests/test_pure_logic.lua`'s `readFile`/`srcPath` targets, `util/.luacheckrc`'s per-file override key, `ci.yml`'s version-sync grep target, `README.md`, and `.agents/AGENTS.md`.
  - **`TAC/` Folder Restored (repo root is no longer flat)**: Superseding the flatten above, the repo root now holds a single `TAC/` folder containing `init.lua`, `tac.lua`, `config/`, `tests/`, `util/`, `README.md`, and `CHANGELOG.md` — i.e. `TAC/` *is* the exact folder to drop into `<MacroQuest Root>/lua/`, whether from a source checkout or a release download. This replaces the prior model where the release zip only synthesized a `TAC/`-prefixed archive from an otherwise-flat repo.
  - **`tac.lua` Data-Path Lookup Updated**: `tac.lua` now checks for `config/triune_data.lua` beside itself first (i.e. `TAC/config/triune_data.lua`), before falling back to your MQ config directory or `mq.luaDir` for legacy/manual placements.
  - **CI/Release Fixes**: `ci.yml`'s syntax/version-sync/luacheck/unit-test steps now all target paths under `TAC/`. `release.yml`'s packaging step simplified to a single `git archive` of `TAC/` (excluding `TAC/tests` and `TAC/util` via pathspec) — the prior staging-directory workaround is no longer needed since `TAC/` already contains exactly what should ship. `.gitattributes`' `export-ignore` entries for `util`/`tests` moved to `TAC/util`/`TAC/tests`.
  - **`config/triune_data.lua` Recovered**: This file had gone missing from the checkout (its `config/` folder was lost, likely during the `git rm -r TAC` step of the flatten above, before its paired `git mv` could complete). Recovered and re-added at `TAC/config/triune_data.lua` — confirmed byte-identical to the live copy in the local MacroQuest install.
